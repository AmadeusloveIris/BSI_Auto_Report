...
class Report(object):
    ...
    def get_IL_loction(self, select_chain):
        ilinterface = select_chain.find_all('div', {'class': 'il-stats subsection unbreakable'})[0].table
        tIL_format = []

        def conserved_ornot(percenti,percentl,region):
            if percenti*percentl==0 and percenti+percentl!=0 and region=='CONSTANT':
                return 'Yes'
            else:
                return 'N/A'

        for idx, tr in enumerate(ilinterface.find_all('tr')):
            if idx != 0:
                tds = tr.find_all('td')
                tIL_format.append({
                    'region': tds[0].contents[0],
                    'position': tds[1].contents[0].split('@')[1],
                    'differentiation': tds[1].contents[0].split('@')[0],
                    'conserved': conserved_ornot(int(tds[2].contents[0]),int(tds[3].contents[0]),tds[0].contents[0]),
                    'psmpepsin': tds[4].contents[0],
                    'psmchymo': tds[5].contents[0],
                    'psmIEThcD': tds[6].contents[0],
                    'psmLEThcD': tds[7].contents[0],
                    'confidence': tds[8].contents[0]
                })
        return tIL_format
